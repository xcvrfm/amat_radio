...
fn write_to_file(rel_path: String, data: BitArray) {
  use out_stream <- result.try(file_stream.open_write(rel_path))
  use _ <- result.try(file_stream.write_bytes(out_stream, data))
  use _ <- result.try(file_stream.close(out_stream))
  Ok(Nil)
}

/// Downloads the current SQL schema to a file at rel_path
pub fn fetch_sql_schema(rel_path: String) {
  let assert Ok(btz) =
    urls.sql_schema_url
    |> fetch_data
...
pub fn convert_sql_schema_to_postgres(rel_path_in: String, rel_path_out: String) {
  use in_stream <- result.try(file_stream.open_read(rel_path_in))
  use out_stream <- result.try(file_stream.open_write(rel_path_out))
  let result = sql_schema.process_sql_schema(in_stream, out_stream)
  use _ <- result.try(file_stream.close(in_stream))
  use _ <- result.try(file_stream.close(out_stream))
  result
}
...
pub fn fetch_weekly_application_records(rel_path: String) {
  let assert Ok(btz) =
...
pub fn fetch_weekly_license_records(rel_path: String) {
  let assert Ok(btz) =
